...
export default function App() {
  ...
  // ── Navigation with back-button history ──
  const navigateTo = (newView) => {
    setViewHistory(prev => [...prev.slice(-9), view]); // keep last 10
    setView(newView);
  };
  const goBack = () => {
    if (viewHistory.length === 0) { setView("dashboard"); return; }
  ...
  function addLogisticsItem() {
    ...
    const entry = {
      ...
      costs: [],
      docs: { "Expense Receipts": [], "Purchase Receipt": [], "Tax Exemption Form": [] },
    };
  ...
  async function moveToInventory(id) {
    ...
    const inventoryEntry = {
      ...
      tradeOnPoNumber: "", tradeAllowance: null,
      costs: item.costs || [],
      docs: (item.docs && Object.keys(item.docs).length > 0) ? item.docs : docFolders,
      dateAdded: new Date().toISOString().slice(0,10),
...
<\/script></body></html>`;
  ...
  const renderReport = () => {
    ...
    // ── Report PIN lock screen ──
    if (!reportPinUnlocked) {
      return (
        <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",minHeight:"60vh",gap:24}}>
          <div style={{textAlign:"center"}}>
            <div style={{fontSize:44,marginBottom:8}}>📊</div>
            <h2 style={{margin:"0 0 4px",fontSize:22,fontWeight:800,color:"#edf2fc"}}>Monthly Report</h2>
            <p style={{margin:0,color:"#7a8aaa",fontSize:13}}>Enter PIN to view sales data</p>
          </div>
          <div style={{display:"flex",gap:14}}>
            {[0,1,2,3].map(i=>(
              <div key={i} style={{width:18,height:18,borderRadius:"50%",
                background:reportPinError?"#f87171":reportPinInput.length>i?"#c9a227":"transparent",
                border:`2px solid ${reportPinError?"#f87171":"#c9a227"}`,transition:"all 0.15s"}}/>
            ))}
          </div>
          {reportPinError && <div style={{color:"#f87171",fontSize:13,fontWeight:600,marginTop:-12}}>Incorrect PIN</div>}
            ...
            {[1,2,3,4,5,6,7,8,9,"",0,"⌫"].map((k,i)=>(
              ...
              <button key={i}
                ...
                style={{height:64,borderRadius:12,background:k==="⌫"?"transparent":"#1e2235",
                  ...
                  color:reportPinError?"#f87171":"#edf2fc",
                  fontSize:k==="⌫"?22:24,fontWeight:700,cursor:"pointer",touchAction:"manipulation"}}>
                {k}
              </button>
            ))}
          </div>
    ...
    return (
      <div>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}>
          <h2 style={{ margin:0, fontSize:20, fontWeight:800, color:"#edf2fc" }}>Monthly Sales Report</h2>
          <button style={{...S.btn("danger"),fontSize:12}} onClick={()=>{setReportPinUnlocked(false);setReportPinInput("");}}>🔒 Lock Report</button>
        </div>
          ...
          logisticsItems.map((item, idx) => {
            ...
            return (
              <div key={item.id} style={{background:bgColor, border:`2px solid ${borderColor}`, borderRadius:12, marginBottom:12, padding:16, transition:"all 0.2s"}}>
                ...
                {/* Document folders — same as inventory */}
                <div style={{marginTop:12, borderTop:"1px solid #2a3055", paddingTop:12}}>
                  <div style={{fontSize:11, color:"#c9a227", textTransform:"uppercase", letterSpacing:"0.1em", fontWeight:700, marginBottom:8}}>Documents</div>
                  <div style={{display:"flex", gap:6, marginBottom:10, flexWrap:"wrap"}}>
                    {DOC_FOLDERS.map(f => {
                      const count = ((item.docs||{})[f]||[]).length;
                      return (
                        <button key={f}
                          onClick={()=>setLogActiveFolder({id:item.id, folder:f})}
                          style={{fontSize:11, padding:"3px 10px", borderRadius:5, cursor:"pointer", touchAction:"manipulation",
                            background: (logActiveFolder?.id===item.id && logActiveFolder?.folder===f) ? "#c9a227" : "#1e2235",
                            color: (logActiveFolder?.id===item.id && logActiveFolder?.folder===f) ? "#111" : "#8a9aba",
                            border: `1px solid ${(logActiveFolder?.id===item.id && logActiveFolder?.folder===f) ? "#c9a227" : "#2a3055"}`}}>
                          {f.split(" ")[0]} {count > 0 ? `(${count})` : ""}
                        </button>
                      );
                    })}
                  </div>
                    ...
                    <div>
                      <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(120px,1fr))", gap:8, marginBottom:8}}>
                        {((item.docs||{})[logActiveFolder.folder]||[]).map(d => (
                          <div key={d.id} style={{background:"#111520", border:"1px solid #2e3a58", borderRadius:7, overflow:"hidden"}}>
                            {(d.thumb||d.viewImg)?.startsWith("data:image")
                              ? <img src={d.thumb||d.viewImg} alt={d.name} style={{width:"100%",height:70,objectFit:"cover",display:"block",cursor:"pointer"}}
                                  onClick={()=>{ const s=d.viewImg||d.thumb; if(s) setShowDocViewer({src:s,name:d.name}); }} />
                              : <div style={{width:"100%",height:70,background:"#1e2436",display:"flex",alignItems:"center",justifyContent:"center",fontSize:24}}>📄</div>
                            }
                            <div style={{padding:"4px 6px", display:"flex", justifyContent:"space-between", alignItems:"center"}}>
                              <span style={{fontSize:10,color:"#6a7a9a",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:80}}>{d.name}</span>
                              <button onClick={()=>{
                      ...
                      <label style={{...S.btn("primary"), cursor:"pointer", display:"inline-block", padding:"6px 12px", fontSize:11}}>
                        + Upload
      ...
      <div>
        ...
        {!invoiceEq ? (
          ...
          <div style={{textAlign:"center",marginTop:16}}>
            <button
              style={{...S.btn("primary"), padding:"12px 32px", fontSize:15, display:"inline-flex", alignItems:"center", gap:8}}
              onClick={()=>{
                const el = document.getElementById("rrte-invoice");
                if (!el) return;
                const w = window.open("","_blank","width=820,height=1000");
                if (!w) return;
                w.document.write(`<!DOCTYPE html><html><head><title>Invoice</title><style>*{margin:0;padding:0;box-sizing:border-box;}body{font-family:Inter,system-ui,sans-serif;}@media print{body{-webkit-print-color-adjust:exact;print-color-adjust:exact;}}</style></head><body>${el.outerHTML}</body></html>`);
                w.document.close();
      ...
      <div>
        ...
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:20, flexWrap:"wrap", gap:10}}>
          ...
          <div style={{display:"flex", gap:8, flexWrap:"wrap", alignItems:"center"}}>
            <div style={{background:"#1e2235", border:"1px solid #2a3055", borderRadius:8, padding:"8px 12px"}}>
              <div style={{fontSize:11, color:"#c9a227", fontWeight:700, marginBottom:6}}>Report PIN</div>
              <div style={{display:"flex", gap:6, alignItems:"center"}}>
                <input
                  style={{...S.input, width:90, fontSize:14, letterSpacing:"0.2em"}}
                  type="text"
                  maxLength={4}
                  placeholder="PIN"
                  defaultValue={reportPin}
                  onBlur={e=>{ if(e.target.value.length===4) setReportPin(e.target.value); }}
                />
                <span style={{fontSize:11, color:"#4a5a7a"}}>4 digits</span>
              </div>
            </div>
  ...
  return (
    <div style={S.app}>
      ...
      <div style={{...S.header, height:70}}>
        <div style={{...S.logoWrap, cursor:"pointer"}} onClick={()=>setView("dashboard")} title="Go to Dashboard">
          <img src="data:image/webp;base64," alt="Red River Tractor & Equipment" style={{height:54, width:"auto", objectFit:"contain", cursor:"pointer"}} onClick={()=>{setViewHistory([]); setView("dashboard");}} />
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {viewHistory.length > 0 && (
            <button onClick={goBack} style={{background:"none",border:"1px solid #2a3055",borderRadius:8,color:"#8a9aba",fontSize:13,padding:"4px 10px",cursor:"pointer",touchAction:"manipulation",fontWeight:600}}>
              ← Back
            </button>
          )}
          {storageLoaded && (
        ...
        <div style={{display:"flex", alignItems:"center", gap:4}}>
          {view !== "dashboard" && (
            <button onClick={goBack} style={{background:"none", border:"1px solid #2a3055", borderRadius:6, color:"#8a9aba", padding:"5px 12px", cursor:"pointer", fontSize:13, touchAction:"manipulation", marginRight:4}}>
              ← Back
            </button>
          )}
        </div>
